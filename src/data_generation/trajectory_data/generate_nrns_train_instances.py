...
def build_single_path(sim, episode_id):
    for _ in range(TEST_POINTS):
        ...
        except (GreedyFollowerError, TypeError) as e:
            continue
...
def get_not_done(train_scenes, feats_dir):
    #Determine if number of feats in  matches
    ret = []
    for scene in train_scenes:
        pts = glob(feats_dir + scene + "*")
        if len(pts) < PATHS_PER_HOUSE:
            ret.append(scene)
    return ret
...
if __name__ == "__main__":
    ...
    #scenes_train Y
    #scenes_test
    #scenes_valUnseen.txt
    train_scene_file = data_splits + "scenes_valUnseen.txt"
    with open(train_scene_file) as f:
        train_scenes = sorted([line.rstrip() for line in f])

    submitit_log_dir = "./log_test/"
#    train_scenes = train_scenes[1:37]
#    train_scenes = train_scenes[37:]
#    train_scenes = get_not_done(train_scenes, save_dir + "feats/")
    for house in tqdm(train_scenes):
