...
class Agent:
    def __init__(
        ...
        self.reached_goal_step = self.switch_index
        ...
        self.super_glue = super_glue
        self.super_glue.eval()
        self.matcher = matcher
        self.matcher.eval()
#        self.ddppo = ddppo
#        self.ddppo.eval()
#        self.ddppo_hidden = ddppo_hidden
    ...
    def self_loop(self, node_id):
        ...
        self.graph.edges[node_id, node_id]["attr"] = edge_attr.squeeze(
            0).squeeze(0)
    ...
    def add_node(
        ...
        self.node_poses = torch.cat(
            [self.node_poses, torch.tensor(
                postion, dtype=torch.float).unsqueeze(0)],
            dim=0,
        )
        rotation = quaternion.as_float_array(
            quaternion.from_rotation_matrix(rotation))
        self.node_rots = torch.cat(
            [self.node_rots, torch.tensor(
                rotation, dtype=torch.float).unsqueeze(0)],
            dim=0,
        )
        if feat is not None:
            ...
            feat = torch.Tensor(
                (np.zeros(self.node_feat_size) + 0.5)).unsqueeze(0)
            self.node_feats = torch.cat([self.node_feats, feat], dim=0)
    ...
    def update_agent(self, next_node, pose=None, rotation=None, obs=None):
        ...
        self.rgb_img = obs["rgb"][:, :, :3]
        self.depth_img = obs["depth"]
        self.node_feats[self.current_node, :] = get_res_feats(
            self.rgb_img, self.resnet)
    ...
    def take_step(self, action):
        ...
        if action == "forward":
            ...
            if self.actuation_noise:
                obs = self.sim.step(
                    HabitatSimActions.NOISY_FORWARD)  # noisy forward
            else:
            ...
            if self.actuation_noise:
                obs = self.sim.step(
                    HabitatSimActions.NOISY_RIGHT)  # noisy right
            else:
        ...
        rotation = quaternion.as_float_array(
            self.sim.get_agent_state().rotation)
        if self.pose_noise:
            pose = self.noisy_sensor.get_noisy_pose(
                action, previous_pose, pose)
