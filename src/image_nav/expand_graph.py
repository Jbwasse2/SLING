...
def add_ghosts(agent, ghost_nodes):
    ...
    for node in ghost_nodes:
        #Check if ghost node already exists with this position/rotation
        node_pos = node[0:3, 3]
        node_rotation = quaternion.as_float_array(
            quaternion.from_rotation_matrix(node[0:3, 0:3])
        )
        pose = tuple(np.concatenate([node_pos, node_rotation]))
        if agent.args.dont_reuse_poses:
            if pose in agent.ghost_poses:
                continue
            else:
                agent.ghost_poses[pose] = True
        navigable, node[0:3, 3] = testPoint(node[0:3, 3], agent)
        if not navigable:
            continue
        localized, location = agent.localize(node_pos, node_rotation)
        if localized:
            add_ghost_edge(agent, location)
#            addEdge.append(location)
        else:
            add_ghost_node(agent, node)
#            addNode.append(node)
