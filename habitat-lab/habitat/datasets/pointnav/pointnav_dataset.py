...
class PointNavDatasetV1(Dataset):
    ...
    def __init__(self, config: Optional[Config] = None) -> None:
        ...
        with gzip.open(datasetfile_path, "rt") as f:
            self.from_json(f.read(), scenes_dir=config.SCENES_DIR, difficulty=config.DIFFICULTY)
    ...
    def from_json(
        self, json_str: str, scenes_dir: Optional[str] = None, difficulty: Optional[str] = 'easy'
    ) -> None:
        ...
        for episode in deserialized["episodes"]:
            if difficulty != 'all':
                if episode['info']['difficulty'] != difficulty:
                    continue
