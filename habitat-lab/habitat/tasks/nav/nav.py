...
class NavigationGoal:
    ...
    rotation: Optional[List[float]] = attr.ib(default=None, validator=not_none_validator)
...
class ImageGoalSensor(Sensor):
    ...
    def _get_pointnav_episode_image_goal(self, episode: NavigationEpisode):
        ...
        source_rotation = np.array(episode.goals[0].rotation, dtype=np.float32)
...
class EpisodicCompassSensor(HeadingSensor):
    ...
    def get_observation(
        ...
        agent_state = self._sim.get_agent_state()
        m = agent_state.rotation
        return np.array([m.w, m.x, m.y, m.z])
        rotation_world_agent = agent_state.rotation
        rotation_world_start = quaternion_from_coeff(episode.start_rotation)
