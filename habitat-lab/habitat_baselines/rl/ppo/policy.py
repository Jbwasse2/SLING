...
from habitat_baselines.rl.ppo.lastmile import get_target
...
class Policy(nn.Module, metaclass=abc.ABCMeta):
    ...
    def act(
        ...
        # Determine if end of exploration
        targets = []
        for rgb, depth, imageGoal in zip(
           observations["rgb"],
           observations["depth"],
           observations["imagegoal"],
        ):
           img1 = rgb.cpu().detach().numpy()
           d1 = depth.cpu().detach().numpy()
           img2 = imageGoal.cpu().detach().numpy()
           N = 128 / 2
           K = np.array([[N, 0, N], [0, N, N], [0, 0, 1]])
           target = get_target(img1, img2, 10 * d1, K, device, superglue)
           if not isinstance(target, np.ndarray):
               targets.append(None)
           else:
               targets.append(target)

        def reshape_mine(inp):
            inp = torch.swapaxes(inp, 1, 3)
            inp = F.interpolate(inp, size=128)
            inp = torch.swapaxes(inp, 1, 3)
            return inp

        observations["rgb"] = reshape_mine(observations["rgb"])
        observations["imagegoal"] = reshape_mine(observations["imagegoal"])
        ...
        return value, action, action_log_probs, rnn_hidden_states, targets
