...
import wandb
from habitat_baselines.rl.ppo.validity_func.local_nav import LocalAgent, loop_nav
...
from habitat_baselines.rl.ppo.matching_superglue import Matching
...
from ssl_baselines.common.data_augmentations import compose_augmentations
from ssl_baselines.rl.ddppo.policy import resnet_policy  # noqa: F401
from ssl_baselines.rl.ppo import PPO
from ssl_baselines.rl.ddppo.algo import DDPPO


@baseline_registry.register_trainer(name="ssl_ddppo")
class SSLPPOTrainer(BaseRLTrainer):
    ...
    def __init__(self, config=None):
        ...
        self.superglue = self.get_superglue()

    def get_superglue(self):
        config = {
            "superpoint": {
                "nms_radius": 4,
    ...
    def _setup_actor_critic_agent(self, ppo_cfg: Config) -> None:
        ...
        self.obs_augmentations = compose_augmentations(
            self.config.RL.POLICY.OBS_AUGMENTATIONS,
            self.config.RL.POLICY.OBS_AUGMENTATIONS_PARAMS,
        )
        ...
        if self.config.RL.DDPPO.pretrained:
            ...
            state_dict = self.load_weights(pretrained_state)
            msg = self.actor_critic.net.visual_encoder.load_state_dict(
                state_dict, strict=False
            )
            logger.info(
                "Pretrained weights found at {} and loaded with msg: {}".format(
                    self.config.RL.DDPPO.pretrained_weights, msg
                )
            )

        if self.config.RL.DDPPO.pretrained_goal_encoder:
            state_dict = self.load_weights(pretrained_state)
            self.actor_critic.net.goal_visual_encoder.load_state_dict(
                state_dict, strict=False
            )

        if not self.config.RL.DDPPO.train_encoder:
            self._static_encoder = True
            for param in self.actor_critic.net.visual_encoder.backbone.parameters():
                param.requires_grad_(False)

        if not self.config.RL.DDPPO.train_goal_encoder:
            for (
                param
            ) in self.actor_critic.net.goal_visual_encoder.backbone.parameters():
                param.requires_grad_(False)
        ...
        pytorch_total_params = sum(
            p.numel() for p in self.actor_critic.net.parameters()
        )
        pytorch_trainable_params = sum(
            p.numel() for p in self.actor_critic.net.parameters() if p.requires_grad
        )
        logger.info(
            "Total number of: Parameters: {}   Trainable Parameters {}".format(
                pytorch_total_params, pytorch_trainable_params
            )
        )
        logger.info(
            "Percent Trainable Parameters: {}%".format(
                round(pytorch_trainable_params / pytorch_total_params * 100, 2)
            )
        )

    def load_weights(self, pretrained_state):
        if self.config.RL.DDPPO.pretraining_type == "dino":
            state_dict = pretrained_state["teacher"]
            state_dict = {k.replace("module.", ""): v for k, v in state_dict.items()}
        elif self.config.RL.DDPPO.pretraining_type == "moco":
            state_dict = pretrained_state["state_dict"]
            state_dict = {k: v for k, v in state_dict.items() if "encoder_k" not in k}
            state_dict = {
                k.replace("module.encoder_q", "backbone"): v
                for k, v in state_dict.items()
            }
        else:
            raise ValueError(
                f"Unknown pretraining type: {self.config.RL.DDPPO.pretraining_type}"
            )
    ...
    def _init_train(self):
        ...
        if (self._is_distributed and rank0_only()) or (not self._is_distributed):
            ...
            if resume_state is not None:
                ...
                wandb.init(
                    group=group_name,
                    job_type=str(self.config.TASK_CONFIG.SEED),
                    id=wandb_id,
                    project="ddppo_training",
                    config=self.config,
            ...
            wandb.run.name = (
                self.config.WANDB_NAME + "_" + str(self.config.TASK_CONFIG.SEED)
            )
            wandb.run.save()
        ...
        print("Number of envs for training: {}".format(self.envs.number_of_episodes))
    ...
    def eval(self) -> None:
        ...
        Returns:
            None
        """
        wandb_filename = os.path.join(
            self.config.TENSORBOARD_DIR,
            "wandb_id_eval_" + str(self.config.EVAL.SPLIT) + ".txt",
        )
        ...
        if os.path.exists(wandb_filename):
            with open(wandb_filename, "r") as file:
                wandb_id = file.read().rstrip("\n")
            wandb.init(
                group=self.config.WANDB_NAME,
                job_type=str(self.config.TASK_CONFIG.SEED),
                id=wandb_id,
                project="ddppo_testing",
                config=self.config,
                mode=self.config.WANDB_MODE,
                resume="must",
            )
        ...
        wandb.run.name = (
            ...
            + "_"
            + str(self.config.EVAL.SPLIT)
            + "_"
            + str(self.config.TASK_CONFIG.SEED)
        )
        wandb.run.save()
            ...
            if os.path.isfile(self.config.EVAL_CKPT_PATH_DIR):
                ...
                while True:
                    ...
                    if self.config.NUM_CHECKPOINTS - 1 == prev_ckpt_ind:
                        self.config.defrost()
                        self.config.TEST_EPISODE_COUNT = -1
                        self.config.freeze()
                        self._eval_checkpoint(
    ...
    def _eval_checkpoint(
        ...
        while len(stats_episodes) < number_of_eval_episodes and self.envs.num_envs > 0:
            ...
            if isinstance(targets[0], np.ndarray):
                images = np.hstack([observations[0]['rgb'], observations[0]['imagegoal']])
                print("TARGET = " + str(targets))
                outputs, images = self.localnav(
                    targets[0][0],
                    targets[0][1],
                    observations[0],
                    0,
...
#                rgb_frames[0].extend(images)
                if isinstance(observations, list) and isinstance(outputs, list):
                    observations, rewards_l, dones, infos = [list(x) for x in zip(*outputs)]
                    images = np.hstack([observations[0]['rgb'], observations[0]['imagegoal']])
                    actions[0] = 0
                    skip = True
            ...
            for i in range(n_envs):
                ...
                if not not_done_masks[i].item():
                    # Check if running last mile nav
