...
class PPO(nn.Module):
    def __init__(
        ...
        # use different lr for resnet and other networks
        resnet_params, non_resnet_params = [], []
        for name, param in self.actor_critic.named_parameters():
            if param.requires_grad:
                if "net.visual_encoder.backbone" in name or \
                        "net.goal_visual_encoder.backbone" in name:
                    resnet_params.append(param)
                else:
                    non_resnet_params.append(param)
                
        self.optimizer = optim.Adam(
            [
                {"params": resnet_params, "lr": resnet_lr},
                {"params": non_resnet_params, "lr": lr},
            ],
            lr=lr,
            eps=eps,
