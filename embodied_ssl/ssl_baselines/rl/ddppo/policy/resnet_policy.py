...
from ssl_baselines.rl.ddppo.policy import resnet_hab, resnet_bn, resnet_gn
from ssl_baselines.rl.ddppo.multiview_sensors import ImageGoal360Sensor, RGB360Sensor
from ssl_baselines.rl.ddppo.rotation_reward import ImageGoalRotationSensor


@baseline_registry.register_policy
class PointNavResNetPolicy2(Policy):
    def __init__(
        self,
    ...
    def from_config(
        ...
        return cls(
            ...
            resnet_baseplanes=config.RL.PPO.resnet_baseplanes,
            backbone=config.RL.DDPPO.backbone,
            normalize_visual_inputs="rgb" in observation_space.spaces
            or "rgb_360" in observation_space.spaces,
            force_blind_policy=config.FORCE_BLIND_POLICY,
            policy_config=config.RL.POLICY,
            obs_augmentations=obs_augmentations,
            dropout_prob=config.RL.PPO.dropout_prob,
        )
...
class ResNetEncoder(nn.Module):
    def __init__(
        ...
        if "rgb_360" in observation_space.spaces:
            self._n_input_rgb = observation_space.spaces["rgb_360"].shape[1]
            spatial_size = observation_space.spaces["rgb_360"].shape[2] // 2
            self.num_views = observation_space.spaces["rgb_360"].shape[0]
            assert self.num_views > 1, "Need at least 2 views for 360 camera"
    ...
    def forward(self, observations: Dict[str, torch.Tensor]) -> torch.Tensor:  # type: ignore
        ...
        if self._n_input_rgb > 0:
            if self.num_views == 1:
                ...
                rgb_observations = observations["rgb_360"]
                # [BATCH x VIEWS x CHANNEL x HEIGHT X WIDTH]
                bs, v, c, w, h = rgb_observations.shape
                rgb_observations = (
                    rgb_observations.reshape([bs * v, c, w, h]).float() / 255.0
                )
...
#            rnn_input_size += 32
        ...
        if (
            ImageGoalSensor.cls_uuid in observation_space.spaces
            or ImageGoalRotationSensor.cls_uuid in observation_space.spaces
            or ImageGoal360Sensor.cls_uuid in observation_space.spaces
        ):
            if ImageGoalSensor.cls_uuid in observation_space.spaces:
                goal_observation_space = spaces.Dict(
                    {"rgb": observation_space.spaces[ImageGoalSensor.cls_uuid]}
                )
            elif ImageGoalRotationSensor.cls_uuid in observation_space.spaces:
                goal_observation_space = spaces.Dict(
                    {"rgb": observation_space.spaces[ImageGoalRotationSensor.cls_uuid]}
                )
            else:
                goal_observation_space = spaces.Dict(
                    {"rgb_360": observation_space.spaces[ImageGoal360Sensor.cls_uuid]}
                )
        ...
        if not self.visual_encoder.is_blind:
            ...
            if RGB360Sensor.cls_uuid in observation_space.spaces:
                assert self.visual_encoder.num_views > 1, "Expected at least 2 views."
                self.pano_fc = nn.Linear(
                    self.visual_encoder.num_views * hidden_size, hidden_size
                )
    ...
    def forward(
        ...
        if not self.is_blind:
            ...
            if RGB360Sensor.cls_uuid in observations:
                bs = int(visual_feats.shape[0] / self.visual_encoder.num_views)
                visual_feats = self.pano_fc(visual_feats.view(bs, -1))
...
#            x.append(self.gps_embedding(observations[EpisodicGPSSensor.cls_uuid]))
        ...
        elif ImageGoalRotationSensor.cls_uuid in observations:
            goal_image = observations[ImageGoalRotationSensor.cls_uuid]
            goal_output = self.goal_visual_encoder({"rgb": goal_image})
            x.append(self.goal_visual_fc(goal_output))
        elif ImageGoal360Sensor.cls_uuid in observations:
            goal_image = observations[ImageGoal360Sensor.cls_uuid]
            goal_output = self.goal_visual_encoder({"rgb_360": goal_image})
            goal_output = self.goal_visual_fc(goal_output)
            bs, v = goal_image.shape[:2]
            goal_output = goal_output.reshape([bs, v, -1])
            goal_output = torch.sum(goal_output, dim=1)
...
def save_images(obs):
    # save image observations to disk
    from PIL import Image

    rgb_images = obs["rgb_360"]
