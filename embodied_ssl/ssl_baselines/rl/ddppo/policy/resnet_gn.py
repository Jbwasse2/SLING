...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, ngroups, stride=1, downsample=None, dropout_prob=0.0):
...
class ResNet(nn.Module):
    ...
    def __init__(self, in_channels, base_planes, ngroups, block, layers, dropout_prob=0.0):
        self.inplanes = base_planes
        super(ResNet, self).__init__()
        self.dropout_prob = dropout_prob
        self.conv1 = nn.Conv2d(in_channels, base_planes, kernel_size=7, stride=2, padding=3,
                               bias=False)
    ...
    def _make_layer(self, block, ngroups, planes, blocks, stride=1):
        ...
        layers = []
        layers.append(block(self.inplanes, planes, ngroups, stride, downsample, dropout_prob=self.dropout_prob))
        self.inplanes = planes * block.expansion
        for i in range(1, blocks):
            layers.append(block(self.inplanes, planes, ngroups, dropout_prob=self.dropout_prob))
...
def resnet18(in_channels, base_planes, ngroups, dropout_prob=0.0):
    model = ResNet(in_channels, base_planes, ngroups, BasicBlock, [2, 2, 2, 2], dropout_prob=dropout_prob)
...
def resnet50(in_channels, base_planes, ngroups, dropout_prob=0.0):
    model = ResNet(in_channels, base_planes, ngroups, Bottleneck, [3, 4, 6, 3], dropout_prob=dropout_prob)
    return model

def resnet101(in_channels, base_planes, ngroups, dropout_prob=0.0):
    model = ResNet(in_channels, base_planes, ngroups, Bottleneck, [3, 4, 23, 3], dropout_prob=dropout_prob)
    return model
